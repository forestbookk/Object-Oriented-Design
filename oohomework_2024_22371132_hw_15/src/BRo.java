...
public class BRo {
        ...
        for (Map.Entry<LibraryBookId, Integer> entry : numbers.entrySet()) {
            ...
            if (bookId.isFormal()) {
                ...
                if (brTimes.containsKey(bookId) && brTimes.get(bookId) >= 2) {
                    ...
                    // 该书升级为正式图书的时刻捐献该书的用户信用分额外+2
                    BDc.getInstance().getContributor(bookId).getCredit().contributeUpgrade();
                    ...
                    // 借还若未达到2次，则必须将其送回漂流角。
                    // 放入漂流角
                    BDc.getInstance().addBook(bookId, bookNum, null, false);
                    for (int i = 0; i < bookNum; i++) {
    ...
    /*public void checkEmpty() {
        if (numbers.isEmpty()) {
            ...
            for (Map.Entry<LibraryBookId, Integer> entry : numbers.entrySet()) {
                if (entry.getValue() != 0) {
                    System.out.println("!!!!Still books in BRO");
                    return;
                }
            }
        }
