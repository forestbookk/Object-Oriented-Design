...
public class FightLog {
    ...
    public Boolean ao(ArrayList<String> l, HashMap<String, Adventurer> n, ArrayList<Adventurer> a) {
        ...
        int healthPointDec = n.get(advName).useAnEquipment(Boolean.TRUE, equName);
        if (healthPointDec < 0) {
            //非法武器, 日志无效
            return Boolean.FALSE;
        }
    ...
    public Boolean so(ArrayList<String> l, HashMap<String, Adventurer> n, ArrayList<Adventurer> a) {
        ...
        int healthPointDec = n.get(advName1).useAnEquipment(Boolean.TRUE, equName);
        if (healthPointDec < 0) {
            //非法武器，日志无效
            return Boolean.FALSE;
        }
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (matcher3.find()) {
                ...
                if (ao(logAnalysis, advNameInFight, advInFight)) {
                    ...
                    advNameInFight.get(advName1).storeLog1(logAnalysis);
                    for (int j = 0; j < m; j++) {
                        if (advInFight.get(j).getName().equals(advName1)) {
                            continue;
                        }
