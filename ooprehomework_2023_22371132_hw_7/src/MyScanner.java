...
public class MyScanner {
    ...
    public void scanCmdJ(String[] str, CommandUtil cmdUtil) {
        int advId = Integer.parseInt(str[1]);
        cmdUtil.setAdvId(advId);
        int itemId = Integer.parseInt(str[2]);
        cmdUtil.setItemId(itemId);
        cmdUtil.setName(str[3]);
        String type = str[4];
        cmdUtil.setType(type);
        if (type.equals("EpicEquipment")) {
            double ratio = Double.parseDouble(str[5]);
            cmdUtil.setRatio(ratio);
        } else if (type.equals("CritEquipment")) {
            int critical = Integer.parseInt(str[5]);
            cmdUtil.setCritical(critical);
        } else if (type.equals("ReinforcedBottle") || type.equals("RecoverBottle")) {
            double ratio = Double.parseDouble(str[5]);
            cmdUtil.setRatio(ratio);
        }
    }
